...
$mainDir = explode("homepage.php", $_SERVER['PHP_SELF']);
...
<?php
            ...
            <div class="container content" dir="<?php echo ($langset == "Eng") ? "ltr" : "rtl"; ?>">
                <div class="question contents">
                    ...
                    <form action="?do=Update&Langset=<?php echo ($langset) ?>&record=<?php echo $DBEditrow['question_id'] ?>&returnRevisor=<?php echo $now->format('Y-m-d H:i:s') ?>" method="post" enctype="multipart/form-data">
                        <?php
                        ...
                        <div>
                            ...
                            <?php } ?>
                            <?php if($reviewData['lang']===1){
                                ?>
                            <label for="textarea" class="fw-semibold"><?php echo fn_lang('LANGEXTRA'); ?>
                            <div class="revnote"><?php echo $reviewData['lang_note'] ;?></div>
                            <?php } ?>
                            ...
                            <!-- START OPTIONS -->
                            <!-- START OPTIONS NOTES-->
                            <h3 class="Questions" style="color:z #582f0e"><?php echo fn_lang('QUESTION') ?></h3>
                        ...
                        <div class="d-flex gap-3 col-12 offset-0">
                            ...
                            <div class="d-grid col-3">
                                <input type="submit" id="submitbtn" name="updateDB" value="<?php echo fn_lang('RETURNTOREVISOR') ?>" class="btn btn-primary  btn-lg">
                            </div>
                            <!-- END SUMBIT BTN -->
                            <!-- START CANCEL BTN -->
                            <div class="d-grid col-3">
                                <a href="?do=Dash&L
                                .angset=<?php echo ($langset) ?>" class="btn  btn-warning  btn-lg">Cancel</a>
                            </div>
                            <!-- END CANCEL BTN -->
                            <!-- END BTN AREA -->
                        </div>
        ...
        <div class="question contents">
            ...
            <form action="?do=Save&Langset=<?php echo ($langset); ?>" method="post" enctype="multipart/form-data">
                ...
                <div>
                    ...
                    <!-- END SUBSTEM Image -->
                    <!-- START OPTIONS -->
                    <h3 class="Questions" style="color: #582f0e"><?php echo fn_lang('QUESTION') ?></h3>
                    <div class="Question coloring">
                        <?php for ($i = 1; $i < 5; $i++) {
                            $questionName = ["One", "Two", "Three", "Four"]
                        ?>
                ...
                <div class="d-flex gap-3 col-12 offset-0">
                    <!-- START SUMBIT BTN -->
                    <div class="d-grid col-3">
                        <input type="submit" id="submitBtn" name="saveEdit" value="<?php echo fn_lang('SAVE') ?>" class="btn btn-primary  btn-lg">
                    </div>
                    <!-- END SUMBIT BTN -->
                    <!-- START SEND BTN -->
                    <div class="d-grid col-3">
                        <input type="submit" id="submitBtn" name="send" value="<?php echo fn_lang('SEND') ?>" class="btn btn-success  btn-lg">
                    </div>
                    <!-- END SEND BTN -->
                    <!-- START CANCEL BTN -->
                    <div class="d-grid col-3">
                    ...
                    if ($errCount === 0) {
                        if (isset($_FILES)) {
                            ...
                            if(isset($_GET['returnRevisor'])){
                                ...
                                WHERE question_review.question_id=?");
                                $DBO->execute(array(3,$toTime, $_GET['record']));// return to re revision
    ...
    <?php
            ...
            case 'RevisionSend':
                if (($_SERVER["REQUEST_METHOD"] == "GET" || $_SERVER["REQUEST_METHOD"] == "POST") && isset($_GET['record'])) {
                    ...
                    $RevQ = $con->prepare("UPDATE questions_create SET  question_case=?,cart=?,sent_to_revision_date=? WHERE question_id=?");
                    $RevQ->execute(array(1, 1,date("Y-m-d H:i:s"), $QUESTIONID));
                    //Echo sucess mthods
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                WHERE
                                    questions_create.question_case=?
                                AND
                                    (revisor_one=? OR revisor_two=?)
                                AND
                                    cart=?
                                ORDER BY
                                    creation_date DESC
                                ");
                                $DBEdit->execute(array(1,$_SESSION['teacher_id'],$_SESSION['teacher_id'],1));
                                while ($DBEditrow = $DBEdit->fetch()) {
                                    ...
                                    echo "<td>"."<a href=?do=Revisoredit&record=" . $DBEditrow['question_id'] ."&Revisor=".$revisor."&RevisorQue=".$revisorQue."&Langset=".$langset."&New=1>".fn_lang('REVLINK')."</a></td>";
                                    echo "</tr>";
                                };
                                ?>
                            </tbody>
                        ...
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                ...
                                ORDER BY
                                    creation_date DESC
                                ");
                                $DBEdit->execute(array(1,$_SESSION['teacher_id'],$_SESSION['teacher_id'],0));
                                while ($DBEditrow = $DBEdit->fetch()) {
                                    if($DBEditrow['revisor_one']==$_SESSION['teacher_id']){
                                        $revisorQue=1;
                                        $revisor=$DBEditrow['revisor_one'];
                                    ...
                                    foreach ($DBreviewQuestions as $DBQuestion) {
                                        ...
                                        echo "<td>"."<a href=?do=RevisorReEdit&record=" . $DBQuestion['question_id'] ."&Revisor=".$DBQuestion['revisor_id']."&RevisorQue=".$revisorQue."&Langset=".$langset."&New=0>".fn_lang('REVSTUDY')."</a></td>";
                                    //     echo "</tr>";
                                    };
                                };
                                ?>
...
<div class="container revisor" dir="<?php echo ($langset == "Eng") ? "ltr" : "rtl"; ?>">
    ...
    $DBrow = fn_getRecordsV2("*", "teacher_items", "WHERE teacher_id=? AND account_status=?", array($_SESSION['teacher_id'], 1));
        ...
        if($_GET['New']==1){//1st time to revise a question
            ...
            $DBEdit2=$con->prepare("UPDATE questions_create SET cart=? WHERE question_id=?");
            ...
            ?>
            <div class="question" style='width: 100%;margin:20px;padding:20px;'>
                <form action="?do=AcceptQuestion&Langset=<?php echo ($langset); ?>&Revisor=<?php echo $_SESSION['teacher_id'];?>&QuestionID=<?php echo $DBEditrow['question_id'];?>&ReviewID=<?php echo $result['id'] ;?>" method="POST" encrypted="">
                    <!-- START Revision Page -->
                    ...
                    <section class="outcome">
                        ...
                        <div class="body mb-3">
                            <label class="fw-semibold">
                                ...
                                <input class="checker" type="checkbox" name="depth" id="depth" checked hidden>
                            <!-- START DOK BTN -->
                                <div class="enhanced_bar rev active"
                                style="
                                display: inline-block;
                            ...
                            <!-- START DOK COMMENT -->
                            <div class="body mb-3">
                                <textarea placeholder="<?php echo fn_lang("REVCAUSE");?>" class="form-control" name="depthcheck" id="depthcheck" hidden></textarea>
                            </div>
                            <!-- END DOK COMMENT-->
                        ...
                        <div class="body mb-3">
                            <label class="fw-semibold">
                                ...
                                <input class="checker" type="checkbox" name="reference" id="reference" checked hidden>
                            ...
                            <div class="body mb-3">
                                <textarea placeholder="<?php echo fn_lang("REVCAUSE");?>" class="form-control" name="referencecheck" id="referencecheck" hidden></textarea>
                            </div>
                        ...
                        <div class="body mb-3">
                            ...
                            <!-- END ERROR BTN-->
                            <!-- START ERROR CONTENT-->
                            <input type="text"  id="errkcontent"
                            class="form-control revinp"
                            value=""
                                disabled hidden>
                            <!-- END ERROR CONTENT-->
                        ...
                        <div class="body mb-3">
                            <label class="fw-semibold">
                                ...
                                <input class="checker" type="checkbox" name="identifieronly" id="identifieronly" checked hidden>
                            <!-- START IDENTIFIER BTN -->
                                <div class="enhanced_bar rev active"
                                style="
                                display: inline-block;
                    ...
                    <!-- END OPTIONS SECTION -->
                    <input type="submit" class="btn btn-primary m-3 Accept"name = "accept" value="<?php echo fn_lang("ACCEPT");?>">
                    <input type="submit" class="btn btn-success m-3 Report" name="report" value="<?php echo fn_lang("REPORT");?>">
                    <a href="?do=CancelRevision&Langset=<?php echo ($langset)?>&ReviewID=<?php echo $result['id'];?>&QuestionID=<?php echo $DBEditrow['question_id'];?>"
                    class="btn btn-warning m-3">  <?php echo fn_lang("CANCEL");?></a>
                </form>
            </div>
            <?php
        
        }else{
            //DEBUG START // all else must be deleted - never get through it
            die();
            // Not a new question
            ...
            ?>
            <div class="question" style='width: 100%;margin:20px;padding:20px;'>
                <form action="?do=AcceptQuestion&Langset=<?php echo ($langset); ?>&Revisor=<?php echo $_SESSION['teacher_id'];?>&QuestionID=<?php echo $DBEditrow['question_id'];?>&ReviewID=<?php echo $result['id'] ;?>&ReturnRevisor=<?php echo $$now->format('Y-m-d H:i:s') ;?>" method="POST" encrypted="">
                    <!-- START Revision Page -->
                    <!-- START QUESTION BODY SECTION -->
                    <section class="question">
                        <div class="body mb-3">
                    ...
                    <section class="outcome">
                        ...
                        <div class="body mb-3">
                            <label class="fw-semibold">
                                ...
                                <input class="checker" type="checkbox" name="dok" id="dok" checked hidden>
                            <!-- START DOK BTN -->
                                <div class="enhanced_bar rev active"
                                style="
                                display: inline-block;
                    ...
                    <input type="submit" class="btn btn-primary m-3 Accept"name = "accept" value="<?php echo fn_lang("ACCEPT");?>">
                    <input type="submit" class="btn btn-success m-3 Report" name="report" value="<?php echo fn_lang("REPORT");?>">
                    <a href="?do=Dash&Langset=<?php echo ($langset) ?>" class="btn btn-warning m-3">  <?php echo fn_lang("CANCEL");?></a>
                    <!-- END Revision Page -->
                </form>
            </div>
            <!-- END Revision Page -->
...
<div class="container revisor" dir="<?php echo ($langset == "Eng") ? "ltr" : "rtl"; ?>">
    ...
    $DBrow = fn_getRecordsV2("*", "teacher_items", "WHERE teacher_id=? AND account_status=?", array($_SESSION['teacher_id'], 1));
        ...
        $DBEditrow = $DBEdit->fetch();//record Question line in the variable;
        // change cart after creating the new file of review
        if($_GET['New']==0){// Not a new question just check $_GET // this part is for rereview section
            //start study for question items
            //prepare DB file for edit            
            $DBfile=$con->prepare("SELECT * 
                FROM 
                ...
                ");
            $DBfile->execute(array($_GET['record'],$_GET['Revisor']));
            $result=$DBfile->fetch();//$result stands for review file selected record
            //get Creator
            $DBQuestion=$con->prepare("SELECT * 
            FROM 
                teachers 
            ...
            ");
            $DBQuestion->execute(array($result['teacher_id']));
            $creator=$DBQuestion->fetch();// creator information
            //Get LEARNING OUTCOME
            $DBLO=fn_getRecordsV2("*", "learning_outcomes", "WHERE outcome_id=?", array($result['outcome_id']));
            $LO=$DBLO[0]['unit']." - ".$DBLO[0]['chapter']." - ".$DBLO[0]['item']." - ".$DBLO[0]['content'];//$LO stands for learning outcome
            $now= new Datetime();
            ?>
            <div class="question" style='width: 100%;margin:20px;padding:20px;'>
                <form action="?do=AcceptQuestion&Langset=<?php echo ($langset); ?>&Revisor=<?php echo $_SESSION['teacher_id'];?>&QuestionID=<?php echo $DBEditrow['question_id'];?>&ReviewID=<?php echo $result['id'] ;?>&ReturnRevisor=<?php echo $now->format('Y-m-d H:i:s') ;?>" method="POST" enctype="">
                    <!-- START Revision Page -->
                    <!-- START QUESTION BODY SECTION -->
                    <section class="question">
                        <div class="body mb-3">
                    ...
                    <section class="outcome">
                        ...
                        <div class="body mb-3">
                            ...
                            <div class="body mb-3">
                                ...
                                name="locheck" id="locheck"><?php echo $result['content_aligned_note'];?></textarea>
                            </div>
                            <!-- END LO COMMENT-->
                        </div>
                        <!-- END LO -->
...
case 'CancelRevision':
    ...
    $DBQCreate=$con->prepare("UPDATE questions_create SET cart=? WHERE question_id=?");
    ...
    if ($_SERVER['REQUEST_METHOD'] === "POST") {
        ...
        if(isset($_POST['report'])){
            ...
            SET review_status=:zreport_status,
                ...
                stimulus_adjust=:zrec,
                stimulus_adjust_note=:zrec_comment,
                stem_end=:zstemrev,
            ...
            <?php
            //DEBUG START
            echo "Origin: ".$origin['revisor_time']."<br>";
            echo  "Time: ".$toTime."<br>";
            echo intval($origin['revisor_time']) + $toTime."<br>";
            //DEBUG ENDS
            // fn_redirect("Sending report to Question creator", "success", 30, "?do=Dash&Langset=".$langset);
            ?>
            </div>
            <!-- END PREPARE REPORT -->
            <?php
            ...
            if ($DBRecord['rev_one_approval']==1 && $DBRecord['rev_two_approval']==1){
                //both approved
                $DBO=$con->prepare("UPDATE questions_create SET pilot=? WHERE question_id=?");
                $DBO->execute(array(1,$_GET['QuestionID']));
            }
            ...
            <div class="container">
                ...
                if($_SERVER['REQUEST_METHOD']==='POST'){
                    ...
                    if($errCounter!=0 || $errFileCounter!=0){
                        ...
                        $tempPass=isset($_POST['passwordcheck'])?$_POST['newpassword']:$dbDataTeacher['password'];
                ...
                case 'Delete':
                        ...
                        <?php
                                ...
                                case 'RevisionSend':
                                    if (($_SERVER["REQUEST_METHOD"] == "GET" || $_SERVER["REQUEST_METHOD"] == "POST") && isset($_GET['record'])) {
                                        $QUESTIONID = isset($_GET['record']) && is_numeric($_GET['record']) ? intval($_GET['record']) : 0;
                                        $RevQ = $con->prepare("UPDATE questions_create SET  question_case=?,cart=? WHERE question_id=?");
                                        $RevQ->execute(array(1, 1, $QUESTIONID));
                                        //Echo sucess mthods
                                        $msg = $RevQ->rowCount() . " Record(s) Sent To revision";
                        ?>
                            <!-- Page Heading -->
